...
## 2026-08-26 13:49 — Architecture and plan revised for static pods
Replaced the session 001 extension-service architecture and its 662-line gate-heavy implementation plan with `.journal/002/ARCHITECTURE.md` and `.journal/002/PLAN.md`. The new artifacts make the live-proven `machine.pods` static pod the only production path, retain the Go → Melange → apko image supply chain, define the verified privileged/root/host-network/host-`/dev` baseline, and preserve per-node digest rollout plus Talos-native observability.

The implementation plan now starts from current `master`, treats both spike branches as read-only evidence, cuts over the template in three outcome-oriented phases, repeats the full live matrix with production bytes, and permits privilege reduction only when the same matrix passes. The artifacts explicitly supersede `.journal/001/ARCHITECTURE.md` and `.journal/001/PLAN.md`; those files remain unchanged as historical evidence.

## 2026-08-26 14:43 — Phase 1 production runtime accepted
Implemented and pushed Phase 1 on `feat/static-pod-runtime` at commit `7fc194c`. The clean cutover removes the template command and packages, adds the `incus-guest-agent` command, consumer-owned core ports with generated Mockery mocks, Linux device discovery, transactional ISO-to-tmpfs staging, and subreaper/process-group supervision, plus the canonical Talos static-pod patch and renamed Melange/apko image configuration.
